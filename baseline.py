...
########################################################################################################################
def Kleindessner(X,sexes,nr_centers_per_sex,metric='euclidean',given_centers=np.array([])):
    '''
    INPUT:
    ...
    if m==1:
        ...
        if G.size==0:
            ...
            cluster_centers_rek=Kleindessner(X[new_data_set], sexes_newT,
...
########################################################################################################################
def HeuristicA(X,sexes,nr_centers_per_sex,metric='euclidean',given_centers=np.array([])):
    '''
    INPUT:
    sexes ... integer-vector of length n with entries in 0,...,m-1, where m is the number of groups
    nr_centers_per_sex ... integer-vector of length m with entries in 0,...,k and sum over entries equaling k
    ...
    for ell in np.arange(m):
        ...
        cent_subgroup=k_center_greedy_with_given_centers(X[subgroup].reshape((-1,d)),
                                                               nr_centers_per_sex[ell], metric, given_centers_subgroup)
...
def HeuristicB(X,sexes,nr_centers_per_sex, metric='euclidean',given_centers=np.array([])):
    ...
    if k==0:
        ...
        while kk<k:
            ...
            distance_to_closest = np.amin(np.vstack((distance_to_closest, pairwise_distances(X[new_point].reshape(-1,d), X, metric = metric))), axis=0)
            kk+=1
